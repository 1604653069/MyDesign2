...
public class NetGameFragment extends BaseGameFragment implements INetView, GoBangBoard.PutChessListener, View.OnTouchListener, View.OnClickListener {
    ...
    private void initView(View view) {
        manager = new Manager(getActivity(),getActivity());
        history = new History();
        ...
        player = (TextView) view.findViewById(R.id.tv_player);
    }
        ...
        try {
            ...
            switch (type) {
                case Message.MSG_TYPE_HOST_BEGIN:
                    ...
                    if(Util.mode==1)
                    {
                        player.setText("RedMiNote2  Vs  Wifi玩家");
                    }
                    if(Util.mode==2)
                    {
                        player.setText("RedMiNote2  Vs  蓝牙玩家");
                    }
    ...
    public void onPutChess(int[][] board, int x, int y) {
        if (mIsMePlay && GameJudger.isGameEnd(board, x, y)) {
            ...
            if(mIsHost)
            {
                ...
                history.setMycolor("black");
                history.setHiscolor("white");
            }
            if(Util.mode==1)
            {
                history.setMode("WiFi对战");
            }else if(Util.mode==2)
            {
                history.setMode("蓝牙对战");
            }
            history.setTime(gameTime+"");
